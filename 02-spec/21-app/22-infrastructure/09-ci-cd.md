...
Purpose: lock in every W-class fix from `23-audits/audit-2026-04-19-ai-readiness-score.md` AND the F-FOLDER-OVERVIEW closure from `23-audits/audit-2026-04-19-100-retrospective.md` so the same drift cannot return via a future PR. Runs against `02-spec/21-app/**/*.md` on every PR. Composite of twelve sub-checks, each of which fails the job independently with a precise error.

| Sub-check | Tool | What it asserts | Fix-source it locks |
|---|---|---|---|
| `link-check` | `lychee --offline 02-spec/21-app/**/*.md` | Every relative markdown link resolves to an existing file. | W-5 (broken a11y link). |
...
| `money-units` | `scripts/lint/money-units.ts` | Disallows `amount_minor`, `amount_in_cents`, `priceInCents` anywhere in `02-spec/21-app/`. Only `amount_cents` permitted. | W-10. |
...
| `folder-overview` | `scripts/lint/folder-overview.ts` | Every directory under `02-spec/21-app/` (recursively, excluding hidden dirs and the root itself) MUST contain a file named exactly `00-overview.md`. The file MUST be ≥ 40 lines (proxy for "not a stub") AND MUST contain the headings `## 1. Responsibilities`, `## 2. File-by-file behaviour` (or `behavior`), `## 3. Tasks performed by this folder`, `## 4. What this folder is NOT`, `## 5. Cross-references`. Canonical template: `02-spec/21-app/templates/folder-overview.md`. Allowlist: `scripts/lint/folder-overview.allowlist.txt` (folders explicitly exempt — currently empty; `02-spec/21-app/templates/` is auto-exempt as a non-domain folder). | F-FOLDER-OVERVIEW (retrospective §8.1). |
...
- All scripts are pure read-only on `02-spec/21-app/**`; never modify files.
