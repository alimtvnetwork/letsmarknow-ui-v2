...
> **Method:** Read each new file; for every concrete claim (role, breakpoint, error code, rate limit, storage path, currency, timezone, vendor, environment URL, naming convention), grep the rest of `02-spec/21-app/` for the same concept. Record every mismatch.
