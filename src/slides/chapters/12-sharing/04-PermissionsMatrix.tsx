          ...
          <table className="w-full text-lg">
            <thead>
              <tr className="border-b border-[hsl(var(--slide-border))] bg-[hsl(var(--slide-surface-2))]">
                ...
                {roles.map((r) => (
                  <th key={r} className="px-6 py-4 font-semibold text-center">{r}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {actions.map((a) => (
                <tr key={a.name} className="border-b border-[hsl(var(--slide-border))] last:border-0">
                  <td className="px-6 py-3 text-[hsl(var(--slide-muted))]">{a.name}</td>
                  {a.v.map((ok, i) => (
                    <td key={i} className="px-6 py-3 text-center">
                      {ok ? (
                        <span className="text-[hsl(var(--slide-accent))] text-2xl">●</span>
                      ) : (
                        <span className="text-[hsl(var(--slide-border))] text-2xl">○</span>
                      )}
                    </td>
                  ))}
                </tr>
