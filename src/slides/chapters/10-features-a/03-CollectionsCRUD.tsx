  ...
  return (
    <SlideLayout chapter="Chapter 10 · Capture & Organize" pageLabel="10.03">
      <div className="h-full flex flex-col pt-12">
        ...
        <p className="text-xl text-[hsl(var(--slide-muted))] mb-10">Slug is permanent. Name is cosmetic. Delete is reversible. Move respects Org boundary.</p>

        <div className="space-y-3 flex-1">
          {ops.map((o) => {
            const Icon = o.icon;
            return (
              <div key={o.name} className="rounded-2xl border border-[hsl(var(--slide-border))] bg-[hsl(var(--slide-surface))] px-6 py-5 flex items-center gap-6">
                ...
                <code className="text-sm font-mono px-3 py-1 rounded bg-[hsl(var(--slide-surface-2))] text-[hsl(var(--slide-accent))] w-32 text-center shrink-0">{o.chord}</code>
        ...
        <div className="mt-6 rounded-xl border border-[hsl(var(--slide-border))] bg-[hsl(var(--slide-surface))] p-4 text-sm flex items-center gap-3">
          <span className="text-[hsl(var(--slide-accent))] font-mono text-xs uppercase tracking-widest">Locked rule</span>
          <span className="text-[hsl(var(--slide-fg))]/85">Slug is generated once at creation and never changes. Rename only updates display name. Old links keep resolving forever.</span>
        </div>
      </div>
    </SlideLayout>
  );
