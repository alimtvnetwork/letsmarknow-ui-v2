...
function CellIcon({ value }: { value: Cell }) {
  if (value === "yes")
    return (
      <div className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-[hsl(var(--slide-success)/0.18)]">
        <Check className="w-5 h-5 text-[hsl(var(--slide-success))]" />
      </div>
    );
  if (value === "no")
    return (
      <div className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-[hsl(var(--slide-fg)/0.06)]">
        <X className="w-5 h-5 text-[hsl(var(--slide-muted))]" />
      </div>
    );
  return (
    <div className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-[hsl(var(--slide-warning)/0.18)]">
      ...
      <div className="h-full flex flex-col">
          ...
          <table className="w-full text-base">
            <thead>
              <tr className="border-b border-[hsl(var(--slide-border))] bg-[hsl(var(--slide-fg)/0.04)]">
                ...
                {cols.map((c) => (
                  <th key={c.key} className="px-3 py-4 font-semibold text-center">
                    {c.label}
                  </th>
                ))}
              </tr>
        ...
        <div className="mt-5 flex items-center gap-7 text-sm text-[hsl(var(--slide-muted))]">
          <span className="flex items-center gap-2">
            <CellIcon value="yes" /> Allowed
          </span>
          <span className="flex items-center gap-2">
            <CellIcon value="no" /> Denied
          </span>
          <span className="flex items-center gap-2">
            <CellIcon value="scoped" /> Scoped — only on resources explicitly granted
          </span>
        </div>
      </div>
    </SlideLayout>
