    ...
    <SlideLayout chapter="Chapter 13 · History, Activity & Conflicts" pageLabel="13.05">
      <div className="h-full flex flex-col pt-8">
        ...
        <div className="flex-1 flex items-center justify-center min-h-0">
          <div className="w-full max-w-3xl rounded-2xl border-2 border-[hsl(45_90%_60%_/_0.5)] bg-[hsl(var(--slide-surface))] p-6 shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
              <AlertTriangle className="w-6 h-6 text-[hsl(45_90%_60%)]" />
              <h2 className="text-2xl font-bold">Resolve naming conflict</h2>
            </div>
            <p className="text-[hsl(var(--slide-muted))] mb-4">
              You and Marcus renamed this collection at the same time. Pick one — or write something new.
            </p>

            <div className="space-y-2 mb-4">
              <button className="w-full text-left px-4 py-3 rounded-lg border-2 border-[hsl(var(--slide-accent)_/_0.5)] bg-[hsl(var(--slide-accent)_/_0.08)]">
                <div className="text-xs text-[hsl(var(--slide-muted))] mb-1">Anya wrote (you)</div>
                <div className="text-base font-semibold">Marketing Plan</div>
              </button>
              <button className="w-full text-left px-4 py-3 rounded-lg border border-[hsl(var(--slide-border))] bg-[hsl(var(--slide-surface-2))]">
                <div className="text-xs text-[hsl(var(--slide-muted))] mb-1">Marcus wrote</div>
                <div className="text-base font-semibold">Q2 Marketing Plan</div>
              </button>
              <button className="w-full text-left px-4 py-3 rounded-lg border border-dashed border-[hsl(var(--slide-border))] text-[hsl(var(--slide-muted))]">
                + Write a custom name
              </button>
              <button className="w-full text-left px-4 py-3 rounded-lg border border-dashed border-[hsl(var(--slide-border))] text-[hsl(var(--slide-muted))]">
