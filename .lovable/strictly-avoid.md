...
2. **NO FOLDER NAME `memories/`**: The institutional knowledge folder must ALWAYS be `.lovable/memory/`, NEVER `.lovable/memories/`.
3. **NO CODEBASE / APPLICATION MODIFICATIONS DURING READ-ONLY PHASES**: Never refactor, edit, or write application source code during read memory/onboarding phases (only readme lowercase self-healing and `.lovable/` memory persistence allowed).
...
5. **NO HALLUCINATIONS / GUESSING PAST SPECS**: The repository specs and codebase are the single source of truth. Training data is not. Never invent rules or guess past ambiguities. Open ambiguities must be logged under `.lovable/ambiguous-questions/01-new-ambiguity/`.
